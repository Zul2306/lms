						...
						<div class="form-group">
							...
							<div class="col-sm-3">
								<select class="form-control top-meu-select-property" id="topbar-menu-select-property">
									<?php if (customCompute($fmenus)) foreach ($fmenus as $fmenu) { ?>
	...
	$(document).on('click', '.submit-menu', function() {
		...
		$('ol.sortable > li').each(function(index, value) {
			if (jQuery.inArray($(value).children().attr('data-rand'), flagArray) === -1) {
				...
				arr.push({
					'fmenuID': activeMenuID,
					'menu_typeID': $(value).children().attr('data-type-id'),
					'menu_parentID': '0',
					'menu_pagesID': $(value).children().attr('data-id'),
					'menu_label': navValue,
					'menu_link': link,
					'menu_orderID': loop,
					'menu_rand': $(value).children().attr('data-rand'),
					'menu_rand_parentID': '0'
				});
				loop++;
			...
			$('ol.sortable > li#' + $(value).attr('id') + ' > ol > li').each(function(index2, value2) {
				if (jQuery.inArray($(value2).children().attr('data-rand'), flagArray) === -1) {
					...
					arr.push({
						'fmenuID': activeMenuID,
						'menu_typeID': $(value2).children().attr('data-type-id'),
						'menu_parentID': $(value).children().attr('data-id'),
						'menu_pagesID': $(value2).children().attr('data-id'),
						'menu_label': navValue2,
						'menu_link': link2,
						'menu_orderID': loop,
						'menu_rand': $(value2).children().attr('data-rand'),
						'menu_rand_parentID': $(value).children().attr('data-rand')
					});
					loop++;
				...
				$('ol.sortable > li > ol> li#' + $(value2).attr('id') + ' > ol > li').each(function(index3, value3) {
					if (jQuery.inArray($(value3).children().attr('data-rand'), flagArray) === -1) {
						...
						arr.push({
							'fmenuID': activeMenuID,
							'menu_typeID': $(value3).children().attr('data-type-id'),
							'menu_parentID': $(value2).children().attr('data-id'),
							'menu_pagesID': $(value3).children().attr('data-id'),
							'menu_label': navValue3,
							'menu_link': link3,
							'menu_orderID': loop,
							'menu_rand': $(value3).children().attr('data-rand'),
							'menu_rand_parentID': $(value2).children().attr('data-rand')
						});
						loop++;
